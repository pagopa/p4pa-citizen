...
import it.gov.pagopa.pu.citizen.exception.common.ConflictException;
...
class DebtPositionFacadeServiceImplTest {
  ...
  void verifyNoMoreInteractions() {
    Mockito.verifyNoMoreInteractions(
      ...
      pagedDebtorDebtPositionMapperMock,
  ...
  void givenValidDebtPositionRequestDTOWhenCreateDebtPositionThenOk() {
    ...
    when(organizationRetrieverServiceMock.isCieBroker(brokerId,accessToken)).thenReturn(false);
    when(organizationRetrieverServiceMock.getValidOrganization(requestDTO.getOrganizationId(), brokerId, accessToken)).thenReturn(organization);
    when(debtPositionDTOMapperMock.mapSpontaneousDebtPositionDTO(requestDTO, 1)).thenReturn(debtPosition);
    when(debtPositionServiceMock.createDebtPosition(debtPosition, false, accessToken)).thenReturn(debtPosition);

    PostalIbanVerifyResponse postalIbanVerifyResponse = new PostalIbanVerifyResponse();
    when(installmentFacadeServiceMock.extractPostalIbanVerifyResponse(Mockito.anyList(), any(), Mockito.eq(accessToken))).thenReturn(postalIbanVerifyResponse);

    when(debtPositionResponseDTOMapperMock.map(debtPosition, organization, false, postalIbanVerifyResponse)).thenReturn(expectedResult);
  ...
  void givenInvalidOrganizationIdWhenCreateDebtPositionThenThrowException() {
    ...
    when(organizationRetrieverServiceMock.isCieBroker(brokerId,accessToken)).thenReturn(false);
    when(organizationRetrieverServiceMock.getValidOrganization(requestDTO.getOrganizationId(), brokerId, accessToken)).thenThrow(NotFoundException.class);

    assertThrows(NotFoundException.class, () -> debtPositionFacadeService.createSpontaneousDebtPosition(brokerId, requestDTO, accessToken));
  ...
  void givenCieBrokerIdWhenCreateDebtPositionThenOk() {
    ...
    when(organizationRetrieverServiceMock.isCieBroker(brokerId,accessToken)).thenReturn(true);
    when(cieDebtPositionFacadeServiceMock.createSpontaneousDebtPosition(requestDTO,accessToken)).thenReturn(expectedResult);
  ...
  void whenGetDebtPositionNoticesZipThenOk() {
    ...
    when(printPaymentNoticeServiceMock.generateNotice(Mockito.anyString(), eq(debtPositionDTO), eq(accessToken))).thenReturn(fileResourceDTO);
  ...
  void givenValidDebtPositionWhenGetDebtPositionDetailThenReturnDebtPositionDetail() {
    ...
    when(debtPositionExtendedDTOMapperMock.map(debtPositionDTO, postalIbanVerifyResponse)).thenReturn(expectedResult);
    //when
  ...
  void givenNavWithBrokerCieWhenGetPaymentNoticeThenOk() {
    ...
    when(organizationRetrieverServiceMock.isCieBroker(brokerId,accessToken)).thenReturn(true);
    when(cieDebtPositionFacadeServiceMock.generateNoticeCie(nav, fiscalCode, accessToken))
      .thenReturn(expectedResult);

    FileResourceDTO result = debtPositionFacadeService.getPaymentNotice(fiscalCode, brokerId, organizationId,nav, accessToken);
  ...
  void givenNavWhenGetPaymentNoticeThenOk() {
    ...
    when(organizationRetrieverServiceMock.isCieBroker(brokerId,accessToken)).thenReturn(false);
    Mockito.doNothing().when(organizationRetrieverServiceMock).validateOrganization(organizationId, brokerId, accessToken);
    when(debtPositionServiceMock.getDebtPositionsByOrganizationIdAndNav(organizationId, nav, debtPositionOrigins, accessToken))
      .thenReturn(List.of(debtPositionDTO));
    when(printPaymentNoticeServiceMock.generateNotice(installmentDTOUNPAID.getNav(), debtPositionDTO, accessToken))
      .thenReturn(expectedResult);
  ...
  void givenInvalidDebtPositionOriginWhenGetPaymentNoticeThenValidationException() {
    ...
    when(organizationRetrieverServiceMock.isCieBroker(brokerId,accessToken)).thenReturn(false);
    Mockito.doNothing().when(organizationRetrieverServiceMock).validateOrganization(organizationId, brokerId, accessToken);
    when(debtPositionServiceMock.getDebtPositionsByOrganizationIdAndNav(organizationId, nav, debtPositionOrigins, accessToken)).thenReturn(List.of(debtPositionDTO));

    assertThrows(ValidationException.class,()-> debtPositionFacadeService.getPaymentNotice(fiscalCode, brokerId, organizationId, nav, accessToken));
  ...
  void givenNoMatchingOrganizationIdWhenGetPaymentNoticeThenConflictException() {
    ...
    assertThrows(ConflictException.class,()-> debtPositionFacadeService.getPaymentNotice(fiscalCode, brokerId, organizationId, nav, accessToken));
  ...
  void givenNoDebtPositionWhenGetPaymentNoticeThenReturnNull() {
    ...
    when(organizationRetrieverServiceMock.isCieBroker(brokerId,accessToken)).thenReturn(false);
    Mockito.doNothing().when(organizationRetrieverServiceMock).validateOrganization(organizationId, brokerId, accessToken);
    when(debtPositionServiceMock.getDebtPositionsByOrganizationIdAndNav(organizationId, nav, debtPositionOrigins, accessToken)).thenReturn(List.of());
  ...
  void givenNullOrganizationsWhenGetPagedUnpaidDebtPositionsThenThrowException() {
    ...
    assertThrows(NotFoundException.class, () -> debtPositionFacadeService.getPagedUnpaidDebtPositions(xFiscalCode,brokerId, null, null, pageable, accessToken));
  }
  ...
  void givenValidDebtPositionOverviewWhenGetThenReturnMappedDTO() {
    ...
    when(debtPositionServiceMock.getDebtorDebtPositionOverview(debtPositionId, debtorFiscalCode, organizationId, accessToken))
      .thenReturn(debtPositionDTO);
    when(organizationRetrieverServiceMock.getValidOrganization(organizationId, brokerId, accessToken))
      .thenReturn(org);
    when(receiptServiceMock.getReceiptNoPiiList(Set.of(inst.getReceiptId()),accessToken)).thenReturn(receiptNoPIIList);
    when(
      installmentFacadeServiceMock.extractPostalIbanVerifyResponse(
        Mockito.eq(po.getInstallments()),
        Mockito.any(),
        Mockito.eq(accessToken)
      )
    ).thenReturn(postalIbanVerifyResponse);
